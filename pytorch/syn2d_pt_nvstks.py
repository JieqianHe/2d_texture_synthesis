...
parser.add_argument('--J', type=float, default=4, help='largest scale for defining wavelets')
...
#     def __init__(self, psi_hat_real, psi_hat_imag, second_all = False):
...
def Dealias(x0):
    n = x0.shape[0]
    x_np = x0.data.cpu().numpy()
    x_np = x_np[:,:,0] + 1j * x_np[:,:,1]
    x_hat = np.fft.fft2(x_np)
    kx = np.concatenate((np.arange(int(n/2)), np.arange(int(n/2)) - n/2))
    ky = np.concatenate((np.arange(int(n/2)), np.arange(int(n/2)) - n/2))
    Kx, Ky = np.meshgrid(kx, ky)
    K = Kx**2 + Ky**2
    kcut = (1/3) * np.max(K)   
    ind = np.where(kcut < K)
    x_hat[ind] = 0
    x = np.fft.ifft2(x_hat)
    x = np.concatenate(((np.real(x)).reshape(n,n,1), (np.imag(x)).reshape(n,n,1)), 2)
...
def corner_wave_in_freq(n):
    phi_hat = np.zeros((n,n), dtype = complex)
    kx = np.concatenate((np.arange(int(n/2)), np.arange(int(n/2)) - n/2))
    ky = np.concatenate((np.arange(int(n/2)), np.arange(int(n/2)) - n/2))
    Kx, Ky = np.meshgrid(kx, ky)
    K = Kx**2 + Ky**2
    ...
    ind = np.where(kcut < K)
    phi_hat[ind] = 1
    return np.fft.fftshift(phi_hat)
...
# psi_hat = radial_wavelet_family_freq_2d(n, K, J, Q, a, sigma)
#phi_hat = low_pass_freq(n, sigma_low_pass, sigma_low_pass)

phi_hat = corner_wave_in_freq(n)
psi_hat = np.zeros((n,n,2*K - 1, int(J*Q+1)), dtype = complex)
j = 0
countj = 0
while j <= J:
    countn = 0
    for m in range(1, K+1):
        l = m-1
        psi_hat[:,:,countn,countj] = radial_wavelet_freq_2d(n, m, l, j, a, sigma)
        countn += 1
        if m > 1:
            l = l-1
            psi_hat[:,:,countn,countj] = radial_wavelet_freq_2d(n, m, l, j, a, sigma)
...
if layer2 or cov:
    ...
    if layer2:
      #  scat = scattering_2d_2layers(phi_hat_real, phi_hat_imag, psi_hat_real, psi_hat_imag)
        scat = scattering_2d_2layers(psi_hat_real, psi_hat_imag)
    else:
